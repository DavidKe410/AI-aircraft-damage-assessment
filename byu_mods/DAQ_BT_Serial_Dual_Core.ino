...
float vectors[8][2] = { { .707, .707 },  //Unit vectors to convert distance readings to points relative to projectile Using 8 vectors to keep indexes consistent for the sensors.
                        ...
                        { 0.707, -0.707 } };
float speed = 20;           //m/s
float dist_correction = 1;  // Factor to multiply distances by to correct for incalibration
...
void loop() {
  ...
  for (int i = 1; i <= 6; i++) {
    ...
    if (dist[i].checkForDataReady()) {  //Check the sensor to see if it has data
      ...
      float distance = dist[i].getDistance() * 1e-3 * dist_correction;  //get distance and convert to meters
